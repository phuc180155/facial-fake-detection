...
class DualEfficientViTv4(nn.Module):
    def __init__(self, channels=1280,\
                 ...
                 emb_dim=32, dim_head=64,dropout=0.15,emb_dropout=0.15, architecture='xception_net', version="cross_attention-spatial-cat",weight=0.5, freeze=0, pool='cls'):  
        ...
        self.spatial_extractor = self.get_feature_extractor(architecture=architecture, num_classes=num_classes, in_channels=3)   # efficient_net-b0, return shape (1280, 8, 8) or (1280, 7, 7)
        self.freq_extractor = self.get_feature_extractor(architecture=architecture, num_classes=num_classes, in_channels=1)
    ...
    def get_feature_extractor(self, architecture="efficient_net", pretrained="", unfreeze_blocks=-1, num_classes=1, in_channels=3):
        extractor = None
